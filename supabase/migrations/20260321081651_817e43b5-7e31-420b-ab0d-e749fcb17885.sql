...
-- Analyses table to save results
...
-- User skill progress for adaptive quizzes
...
CREATE POLICY "Users can insert own progress" ON public.user_skill_progress FOR INSERT WITH CHECK (auth.uid() = user_id);
CREATE POLICY "Users can update own progress" ON public.user_skill_progress FOR UPDATE USING (auth.uid() = user_id);
